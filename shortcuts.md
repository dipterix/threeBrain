...
- The shortcut keys are only active when the mouse is on the main canvas
  (not controling GUI nor the side panel)
- Hold `Ctrl`, `Shift`, or `Alt` and drag the brain if you want to fix
  the rotation axis
- If you are using *MacOSX*, `Alt` key is equivalent to `Option [⌥]`,
  and `Ctrl` is equivalent to `Control [^]`
