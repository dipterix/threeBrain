...
* `bce167df (HEAD -> master)` [_`dipterix`_]: `proxy$set_electrode_data` converts color to `Hex` codes instead of passing the color names
* `140d4609 (origin/master, origin/HEAD)` [_`dipterix`_]: Added a active-only mode for outlines so the electrode outlines are shown only when there is data
