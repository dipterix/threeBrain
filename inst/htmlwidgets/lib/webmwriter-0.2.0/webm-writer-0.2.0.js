...
(function() {
    var WebMWriter = function(ArrayBufferDataStream, BlobBuffer) {
         ...
         */
        function renderAsWebP(canvas, quality, is_datauri = false) {
            if( !is_datauri ){
              canvas = canvas.toDataURL('image/webp', {quality: quality});
            }


            return decodeBase64WebPDataURL(canvas);
        }
        ...
        return function(options) {
             ...
             */
            this.addFrame = function(canvas, altwidth, altheight, is_datauri = false) {
                let webP;
                if(!is_datauri){
                    if (writtenHeader) {
                  ...
                  if ( writtenHeader ) {
                        if (altwidth != videoWidth || altheight != videoHeight) {
                            throw "Frame size differs from previous frames" + videoWidth + 'x' + videoHeight + ' | ' + altwidth + '+' + altheight;
                        }
                  } else {
                    videoWidth = altwidth;
                    videoHeight = altheight;

                    writeHeader();
                    writtenHeader = true;
                  }
                  webP = renderAsWebP(canvas, {quality: options.quality}, true);
                }
