as_row_matrix <- function(x, nr, nc = NULL, storage_mode = NULL) {
  ...
  if (length(nc)) {
    stopifnot2(
      ncol(x) == nc,
      msg = sprintf("`%s` must have %d columns", name, nc)
    )
  }
  x
}
...
ElectrodePrototype <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(type, n_vertices = 4) {
      ...
      stopifnot2(
        isTRUE(n_vertices > 3),
        msg = "Number of vertices must be greater than 3."
      )
    ...
    set_transform = function(m44, byrow = TRUE) {
      stopifnot2(
        is.numeric(m44) && !anyNA(m44),
        msg = "Transform must be a 4x4 numeric matrix with no NA"
      )
      if (is.matrix(m44)) {
        dm <- dim(m44)
    ...
    set_model_control_points = function(
      x,
      y,
      z,
      fixed_point = NULL,
      ...
      stopifnot2(
        nrow(pos) >= 2L,
        msg = "Needs at least 2 unique control points"
      )
      svd <- svd(crossprod(pos))
      if (sum(abs(svd$d > 1e-5)) < 1) {
        stop(
          "The matrix rank of the control points must be at least 2 (you need at least 2 points that are not identical) to calculate the transforms."
        )
      }
      npos <- nrow(pos)
      ...
      if (length(order) > 0) {
        stopifnot2(
          length(order) == npos,
          msg = "`set_model_control_points`: `order` must be integers and/or NAs with size of control points."
        )
      }
    ...
    set_transform_from_points = function(x, y, z, up = NULL) {
      ...
      if (!is.matrix(m_cp)) {
        stop(
          "Please run electrode$set_model_control_points first to set control points on model coordinates"
        )
      }
      ...
      if (!all(is.finite(m44))) {
        if (n >= 3) {
          stop(
            "Cannot calculate the transform from control points. Please make sure the rank of control points is at least 2."
          )
        } else {
          warning(
            "Cannot calculate rotation matrix. More control points are needed."
          )
          return()
        }
      }
      ...
      if (
        ...
        m33 <- solve(m44[1:3, 1:3]) %*%
          self$transform[1:3, 1:3] %*%
          solve(basis)
        m33[m33 < -1] <- -1
        m33[m33 > 1] <- 1
      ...
      if (
        n >= 1 &&
          qr_decomp$rank == 1 &&
          sum(up^2) > 0.5 &&
          sum((self$model_up)^2) > 0.5 &&
          sum((self$model_direction)^2) > 0.5
      ) {
    ...
    set_index = function(index, index_start = NA) {
      ...
      stopifnot2(
        isTRUE(min(index) >= index_start),
        msg = "`index` contains invalid face index"
      )
      stopifnot2(
        isTRUE(max(index) - index_start < private$.n_vertices),
        msg = "`index` contains invalid face index"
      )
    ...
    set_normal = function(normal = "auto") {
      if (is.null(normal)) {
        ...
        if (is.null(private$.normal)) {
          mesh <- ravetools::vcg_update_normals(self$as_mesh3d(
        ...
        private$.normal <- as_row_matrix(
          normal,
          nr = 3,
          nc = private$.n_vertices
        )
      }
    },
    ...
    set_texture_size = function(texture_size) {
      ...
      stopifnot2(
        length(texture_size) == 2 &&
          all(!is.na(texture_size) & texture_size >= 1),
    ...
    set_channel_map = function(channel_map, center_positions) {
      if (!missing(channel_map)) {
        if (!is.null(channel_map)) {
          ...
          channel_map[
            is.na(channel_map) |
              channel_map < 1 |
              channel_map > private$.texture_size
          ] <- NA
        }
        self$.last_texture <- NULL
      ...
      if (missing(center_positions)) {
        if (
          is.matrix(private$.contact_center) &&
            ncol(private$.contact_center) == n_channels
        ) {
      ...
      if (length(center_positions)) {
        ...
        if (ncol(center_positions) != n_channels) {
          stop(sprintf(
            "Number of contacts should be the same as number of channels (%d). Please set texture size and make sure `center_positions` has the same row as `channel_map`.",
            self$n_channels
          ))
        }
      ...
      if (!missing(marker_map)) {
        if (!is.null(marker_map)) {
          ...
          marker_map[
            is.na(marker_map) |
              marker_map < 1 |
              marker_map > private$.texture_size
          ] <- NA
        }
    ...
    set_contact_channels = function(channel_numbers, contact_orders) {
      ...
      n_channels <- self$n_channels
      sel <- !is.na(contact_orders) &
        contact_orders > 0 &
        contact_orders <= n_channels
    ...
    get_contact_positions = function(channels, apply_transform = TRUE) {
      ...
      cpos <- sapply(channels, function(ch) {
        ...
        if (length(sel)) {
          ...
          if (
            apply_transform && length(private$.model_control_point_orders) > 0
          ) {
    ...
    as_mesh3d = function(apply_transform = TRUE, with_texture = TRUE, ...) {
      ...
      if (with_texture && !is.null(self$.last_texture)) {
        texture_file <- file.path(
          tempdir(check = TRUE),
          sprintf("%s.png", private$id)
        )
        grDevices::png(
      ...
      ravetools::rgl_view({
        ...
        lapply(seq_len(nrow(m_cp)), function(ii) {
          ...
          ravetools::rgl_call(
            "text3d",
            pos,
            texts = sprintf("Marker_%d", ii),
            cex = max(0.5, min(radius * 4, 1)),
            pos = 1,
            depth_test = "always"
          )
          ravetools::rgl_call(
            "title3d",
            main = sprintf("Prototype: %s", self$type),
            floating = TRUE
          )
        })
      })
        ...
        tryCatch(
          {
            ...
            self$set_transform_from_points(
              x = tcp[1, ],
              y = tcp[2, ],
              z = tcp[3, ],
              up = li$world_up
    ...
    update_from = function(other) {
      ...
      if (length(other$channel_numbers) == n_channels) {
        ...
        warning(sprintf(
          "The number of electrode contacts [%s] mismatches with prototype [%s]",
          length(other$channel_numbers),
          n_channels
        ))
      }
      if (length(other$world_control_points) >= 6) {
        tcp <- matrix(
          data = other$world_control_points,
          nrow = 3L,
          dimnames = NULL
        )
        tryCatch(
    ...
    validate = function() {
      ...
      if (
        !is.matrix(private$.model_control_points) ||
          nrow(private$.model_control_points) < 2L
      ) {
        warning(
          "Electrode prototype control points (model) must be a matrix with at least 2-3 points. Please use `set_model_control_points` to set them"
        )
      }
      invisible()
    },
    ...
    print = function(..., details = TRUE) {
      ...
      cat(
        ...
        sprintf("  Channels    : %d\n", self$n_channels),
        sprintf(
          "  Anchors     : %d contacts, %d non-contacts\n",
          n_mcp - n_anchors,
          n_anchors
        )
      )
      if (details) {
        cat(
          sep = "",
          sprintf(
            "  Mesh info   : %d vertices, %d triangle faces\n",
            private$.n_vertices,
            length(private$.index) / 3
          ),
  ...
  active = list(
    ...
    contact_sizes = function() {
      if (!length(private$.contact_sizes)) {
        return(rep(0.1, self$n_channels))
      }
      ...
      if (!missing(dir)) {
        if (is.null(dir)) {
          ...
          if (length(dir) != 3L || anyNA(dir)) {
            stop(
              "Electrode direction/normal must have length of 3 and cannot contain NA"
            )
          }
          if (sum(dir^2) > 0) {
            # normalize
    ...
    model_up = function(dir) {
      if (!missing(dir)) {
        if (is.null(dir)) {
          ...
          if (length(dir) != 3L || anyNA(dir)) {
            stop(
              "Electrode `up` direction must have length of 3 and cannot contain NA"
            )
          }
          if (sum(dir^2) > 0) {
            # normalize
    ...
    texture_size = function() {
      private$.texture_size
    },
    channel_map = function() {
      private$.channel_map
    },
    ...
    control_points = function() {
      if (!length(private$.model_control_points)) {
        return(NULL)
      }
    ...
    if (length(f) == 1 && file.exists(f)) {
      ...
      if (file.exists(base_prototype)) {
        ...
        tryCatch(
          ...
          error = function(e) {
            stop(
              "Invalid `base_prototype`; must be instance of one of the followings: `ElectrodePrototype`, json file path, json string, or builtin shape names"
            )
          }
        )
      }
...
load_prototype <- function(type) {
  ...
  if (!length(path)) {
    stop("No such electrode geometry prototype: ", type)
  }
  prototype <- ElectrodePrototype$new("")
