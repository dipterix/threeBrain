...
json_cache <- function(
  ...
  digest = TRUE,
  digest_header = NULL,
  quiet = getOption("threeBrain.quiet", FALSE),
  ...
) {
...
from_json <- function(
  ...
  if (length(from_file) == 1) {
    stopifnot2(
      missing(txt) && file.exists(from_file),
      msg = "If you want to load json from a file, do not specify txt, and make sure from_file exists"
    )
...
read_fs_asc <- function(file) {
  ...
  # Vertices
  vertices <- stringr::str_split(
    src[1 + seq_len(header[1])],
    "[\\ ]+",
    simplify = TRUE
  )
  dim <- dim(vertices)
  ...
  # faces
  faces <- stringr::str_split(
    src[1 + header[1] + seq_len(header[2])],
    "[\\ ]+",
    simplify = TRUE
  )
  dim <- dim(faces)
...
read_fs_m3z <- function(filename) {
  ...
  inds <- outer(
    c(4:1, 8:5, 12:9),
    9 * 4 * (seq_len(width * height * depth) - 1),
    "+"
  )
  ...
  list(
    ...
    get_data = function() {
      ...
      vol <- readBin(
        ...
        n = nv,
        what = rparam$dtype,
        size = rparam$nbytespervox,
        signed = rparam$signed
      )
...
read_fs_labels <- function(path, vertex_number) {
  ...
  vtxct <- readBin(con, "integer", 1, size = 4, endian = "big")
  ...
  tag <- readBin(con, "integer", 1, size = 4, endian = "big")
  ctabversion <- readBin(con, "integer", 1, size = 4, endian = "big")
  maxstruc <- readBin(con, "integer", 1, size = 4, endian = "big")
  len <- readBin(con, "integer", 1, size = 4, endian = "big")
  fname <- readBin(con, "character", n = 1, size = len, endian = "big")
  num_entries <- readBin(con, "integer", n = 1, size = 4, endian = "big")
  ...
  ctab <- do.call(
    ...
    lapply(entries, function(x) {
      data.frame(
        x[c("label", "name", "color_label", "color")],
