...
generate_cortical_parcellation <- function(
  ...
  if (!length(brain$surfaces$sphere.reg)) {
    stop(
      "The FreeSurfer brain object does not have `sphere.reg` surface. Did you finish `recon-all`?"
    )
  }
  ...
  if (!dir.exists(tempolate_path)) {
    message(
      "Template `",
      template_subject,
      "` is missing. Downloading it from RAVE (this is one-time procedure)."
    )
    message(sprintf(
      "Running `threeBrain::download_template_subject(subject_code = '%s')`",
      template_subject
    ))
  ...
  lh_annot_path <- file.path(
    tempolate_path,
    "label",
    sprintf(c("lh.%s.annot", "lh.%s"), annotation)
  )
  rh_annot_path <- file.path(
    tempolate_path,
    "label",
    sprintf(c("rh.%s.annot", "rh.%s"), annotation)
  )
  ...
  if (!length(lh_annot_path)) {
    stop(sprintf(
      "Template `%s` does not have FreeSurfer annotation `%s` for the left hemisphere (lh.%s.annot and lh.%s are missing).",
      template_subject,
      annotation,
      annotation,
      annotation
  ...
  if (!length(rh_annot_path)) {
    stop(sprintf(
      "Template `%s` does not have FreeSurfer annotation `%s` for the right hemisphere (rh.%s.annot and rh.%s are missing).",
      template_subject,
      annotation,
      annotation,
      annotation
  ...
  lh_sphere_reg_path_template <- file.path(
    tempolate_path,
    "surf",
    "lh.sphere.reg"
  )
  rh_sphere_reg_path_template <- file.path(
    tempolate_path,
    "surf",
    "rh.sphere.reg"
  )
  ...
  sphere_reg_template <- freesurferformats::read.fs.surface(
    lh_sphere_reg_path_template
  )
  ...
  if (endsWith(lh_annot_path, ".annot")) {
    ...
    annot_path <- file.path(
      brain$base_path,
      "label",
      sprintf("lh.%s.annot", annotation)
    )
    freesurferformats::write.fs.annot(
      filepath = annot_path,
      num_vertices = as.integer(length(new_label_codes)),
      colortable = annot_template$colortable_df,
      labels_as_colorcodes = new_label_codes
    )
  } else {
