...
check_freesurfer_path <- function(
  ...
  if (dir.exists(fs_subject_folder)) {
    ...
    path_brain_finalsurf <- file.path(
      path_subject,
      "mri",
    ...
    if (!pass_test && check_volume) {
      if (
        any(file.exists(c(
          path_t1,
          path_brain_finalsurf,
          path_brain_automask,
          path_brain_mask
        ))) &&
          file.exists(path_xform)
      ) {
        pass_test <- TRUE
      }
    }
...
electrode_mapped_141 <- function(
  ...
  if (
    isTRUE(is_surface) &&
      length(vertex_number) == 1 &&
      vertex_number >= 0 &&
      length(surf_type) == 1 &&
      !is.na(surf_type) &&
      length(hemisphere) == 1 &&
      hemisphere %in% c("right", "left")
  ) {
...
load_surface_asc_gii <- function(file) {
  if (stringr::str_ends(stringr::str_to_lower(file), "\\.asc")) {
    ...
    tryCatch(
      ...
      error = function(e) {
        stop(
          "Unknown type - Only support ASCII, Gifti, or native FS formats (if freesurferformats is installed)."
        )
      }
    )
  }
...
load_fs_surface <- function(dir, surf_t = "pial", quiet = FALSE) {
  ...
  lh_file <- list.files(
    dir,
    pattern = sprintf("^lh\\.%s\\.(asc|gii|nii)", surf_t),
    full.names = TRUE
  )
  rh_file <- list.files(
    dir,
    pattern = sprintf("^rh\\.%s\\.(asc|gii|nii)", surf_t),
    full.names = TRUE
  )
...
load_141_surface <- function(dir, surf_t = "pial", quiet = FALSE) {
  ...
  lh_file <- list.files(
    dir,
    pattern = sprintf("^std\\.141\\.lh\\.%s\\.(asc|gii|nii)", surf_t),
    full.names = TRUE
  )
  rh_file <- list.files(
    dir,
    pattern = sprintf("^std\\.141\\.rh\\.%s\\.(asc|gii|nii)", surf_t),
    full.names = TRUE
  )
