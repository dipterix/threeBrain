...
localization_module <- function(
  subject_code, fs_path, ct_path, surfaces = "pial", use_141 = TRUE,
  ...
  if(is.null(brain)){
    ...
    if(!isTRUE(file.exists(ct_path))){
      ...
      localize <- local({
        ...
        ct <- threeBrain:::read_nii2( normalizePath(ct_path, mustWork = TRUE) )
        cube <- threeBrain:::reorient_volume( ct$get_data(), brain$Torig )
  ...
  server <- function(input, output, session){
    ...
    shiny::observeEvent(input$load, {
      ...
      tryCatch({
        ...
        shiny::showNotification(p("Error while trying to load from ", infile$name, ": ",
    ...
    output$save <- shiny::downloadHandler(
      ...
      content = function(conn) {
        shiny::showNotification(p("Generating electrode.csv ..."), type = 'message', duration = NULL, id = "save_notif", closeButton = FALSE)
        tbl <- local_reactive$table
...
module <- localization_module(subject_code = "YAB",
                    fs_path = "~/rave_data/data_dir/test1/YAB/fs/",
                    ct_path = "~/Downloads/iELVis_Localization/YAS_CT_aligned1.nii")
module$app
